...
<a class="whatsapp-btn" target="_blank" href="https://wa.me/<?php echo preg_replace('/[^,.0-9]/', '', DE\Helpers::num_whats()); ?>">
...
<?php if (get_the_ID() == 8 || get_the_ID() == 149 || get_the_ID() == 133) : ?>
	<div class="gift">
		<div class="gift__popup">
			...
			<?php if (get_the_ID() == 133) { ?>
				<h4 class="gift__popup-title">
					Erhalten Sie <strong>10% Rabatt</strong> auf Ihre erste Bestellung bis zum 29. Februar!!!
				</h4>
			<?php } else { ?>
				<h4 class="gift__popup-title">
					Sonderangebot für <?php echo DE\Helpers::type_work(); ?>
				</h4>
			<?php } ?>
			<?php if (get_the_ID() == 133) { ?>
				<p class="gift__popup-text">
					Die Anzahl der Geschenke ist limitiert, holen Sie sich Ihr Geschenk schon jetzt!
				</p>
			<?php } else { ?>
				<p class="gift__popup-text">
					Bestellen Sie eine <?php echo DE\Helpers::type_work(); ?> bis zum 29. Februar, erhalten Sie 10% Rabatt auf Ihre erste Bestellung und wählen Sie selbst ein Geschenk!
				</p>
			<?php } ?>
			<?php if (get_the_ID() == 133) { ?>
				<div class="gift__haus-num">10%</div>
			<?php } else { ?>
				<ul class="gift__popup-list">
					<li>
						ExposÉ <span>(bis 3 Seiten)</span>
					</li>
					<li>
						Coaching <span>(30 Min.)</span>
					</li>
					<li>
						Präsentation <span>(7 Folien)</span>
					</li>
				</ul>
			<?php } ?>

			<a class="gift__popup-btn go-to" data-goto=".message" href="#message_u">
				Jetzt erhalten!
			</a>
		</div>
		...
		<div class="gift__box">
			<div class="gift__box-icons gift__animation">
				<div class="gift__box-popup">
					<?php if (get_the_ID() == 133) { ?>
						<p class="gift__box-text">
							Bestellen Sie eine <?php echo DE\Helpers::type_work(); ?> bis zum 31. Dezember und erhalten Sie
					...
					<?php } ?>
					<a class="gift__mobile-btn go-to" data-goto=".message" href="#message_u">
						Jetzt erhalten!
					</a>
				</div>
			</div>
